...
#define FIELD_SET_MASK(reg, field, val) \
    SW_REG_SET_BY_FIELD_U32(hw->msk[((reg) >= 5u) ? ((reg) - 5u) : 0u], val, \
                            reg##_##field##_BOFFSET, reg##_##field##_BLEN)

#define FIELD_GET_MASK(reg, field, val) \
    SW_FIELD_GET_BY_REG_U32(hw->msk[((reg) >= 5u) ? ((reg) - 5u) : 0u], val, \
                            reg##_##field##_BOFFSET, reg##_##field##_BLEN)

#define FIELD_SET_ACTION(reg, field, val) \
    SW_REG_SET_BY_FIELD_U32(hw->act[((reg) >= 10u) ? ((reg) - 10u) : 0u], val, \
                            reg##_##field##_BOFFSET, reg##_##field##_BLEN)

#define FIELD_GET_ACTION(reg, field, val) \
    SW_FIELD_GET_BY_REG_U32(hw->act[((reg) >= 10u) ? ((reg) - 10u) : 0u], val, \
                            reg##_##field##_BOFFSET, reg##_##field##_BLEN)
