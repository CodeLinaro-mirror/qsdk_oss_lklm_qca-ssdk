...
{
	a_uint16_t phy_data = 0;
	phy_data = qca808x_phy_debug_read(dev_id, phy_addr,
...
qca8084_phy_ldo_set(a_uint32_t dev_id, a_uint32_t phy_addr, a_bool_t enable)
{
	...
	if (A_TRUE == other_phy_status) {
		if (enable) {
			...
			rv = qca808x_phy_modify_debug(dev_id, ephy_addr[SSDK_PHYSICAL_PORT1],
				QCA8084_PHY_AFE25_CMN_2_MII,
				QCA8084_PHY_AFE25_LDO_EN, QCA8084_PHY_AFE25_LDO_EN);
			SW_RTN_ON_ERROR(rv);
			rv = qca808x_phy_modify_debug(dev_id, ephy_addr[SSDK_PHYSICAL_PORT2],
				QCA8084_PHY_AFE25_CMN_2_MII,
				QCA8084_PHY_AFE25_LDO_EN, QCA8084_PHY_AFE25_LDO_EN);
			SW_RTN_ON_ERROR(rv);
		} else {
