	...
	a_uint32_t speed, struct qca_phy_priv *priv)
{
	...
	if (port_eee_cfg.lpi_tx_enable == A_FALSE ||
		(priv->lpi_wakeup_timer_force[port_id - 1] == A_TRUE &&
		priv->lpi_sleep_timer_force[port_id - 1] == A_TRUE))
		return SW_OK;
	...
	if (priv->lpi_wakeup_timer_force[port_id - 1] == A_FALSE)
		port_eee_cfg.lpi_wakeup_timer = wakeup_timer_val;

	if (priv->lpi_sleep_timer_force[port_id - 1] == A_FALSE)
	...
	fal_port_eee_cfg_t *port_eee_cfg)
{
	...
	if(port_eee_cfg->lpi_wakeup_timer)
		...
		priv->lpi_wakeup_timer_force[port_id - 1] = A_FALSE;

	if(port_eee_cfg->lpi_sleep_timer)
		priv->lpi_sleep_timer_force[port_id - 1] = A_TRUE;
	else
		priv->lpi_sleep_timer_force[port_id - 1] = A_FALSE;

	adpt_hppe_port_mac_eee_timer_adjust(dev_id, port_id,
		priv->port_old_speed[port_id - 1], priv);
