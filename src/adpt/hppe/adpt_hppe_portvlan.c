		...
		fal_vlan_trans_adv_rule_t *rule, fal_vlan_trans_adv_action_t * action)
{
	...
	if (direction == FAL_PORT_VLAN_INGRESS) {
		...
		rule->c_tagged = in_vlan_xlt_rule.bf.ckey_fmt_0 |
			(in_vlan_xlt_rule.bf.ckey_fmt_1 << SW_FIELD_OFFSET_IN_WORD(XLT_RULE_TBL_CKEY_FMT_OFFSET));
#endif
		...
		action->spcp_xlt = (in_vlan_xlt_action.bf.xlt_spcp_0 |
			(in_vlan_xlt_action.bf.xlt_spcp_1 << SW_FIELD_OFFSET_IN_WORD(XLT_ACTION_TBL_XLT_SPCP_OFFSET)));
#endif
		...
		action->counter_id = (in_vlan_xlt_action.bf.counter_id_0 |
			(in_vlan_xlt_action.bf.counter_id_1 << SW_FIELD_OFFSET_IN_WORD(XLT_ACTION_TBL_COUNTER_ID_OFFSET)));
#endif
		...
		action->spcp_xlt = (eg_vlan_xlt_action.bf.xlt_spcp_0 |
			(eg_vlan_xlt_action.bf.xlt_spcp_1 << SW_FIELD_OFFSET_IN_WORD(EG_VLAN_XLT_ACTION_XLT_SPCP_OFFSET)));
#endif
...
#else
		in_vlan_xlt_rule.bf.ckey_fmt_0 = rule->c_tagged;
		in_vlan_xlt_rule.bf.ckey_fmt_1 = (rule->c_tagged >>
			(SW_FIELD_OFFSET_IN_WORD(XLT_RULE_TBL_CKEY_FMT_OFFSET)));
#endif
...
#else
		in_vlan_xlt_action.bf.xlt_spcp_0 = action->spcp_xlt;
		in_vlan_xlt_action.bf.xlt_spcp_1 = (action->spcp_xlt >>
			(SW_FIELD_OFFSET_IN_WORD(XLT_ACTION_TBL_XLT_SPCP_OFFSET)));
#endif
...
#else
		in_vlan_xlt_action.bf.counter_id_0 = action->counter_id;
		in_vlan_xlt_action.bf.counter_id_1 = (action->counter_id >>
			(SW_FIELD_OFFSET_IN_WORD(XLT_ACTION_TBL_COUNTER_ID_OFFSET)));
#endif
...
#else
		eg_vlan_xlt_action.bf.xlt_spcp_0 = action->spcp_xlt;
		eg_vlan_xlt_action.bf.xlt_spcp_1 = (action->spcp_xlt >>
			(SW_FIELD_OFFSET_IN_WORD(EG_VLAN_XLT_ACTION_XLT_SPCP_OFFSET)));
#endif
...
#if defined(HTTPPE)
		rtn = adpt_httppe_global_qinq_mode_set(dev_id, mode);
		SW_RTN_ON_ERROR(rtn);
#endif
	} else {
...
adpt_hppe_global_qinq_mode_get(a_uint32_t dev_id, fal_global_qinq_mode_t *mode)
{
	...
	if (adpt_chip_type_get(dev_id) == CHIP_HTTPPE) {
#if defined(HTTPPE)
		rtn = adpt_httppe_global_qinq_mode_get(dev_id, mode);
		SW_RTN_ON_ERROR(rtn);
#endif
	} else {
                            ...
                            fal_vlantag_egress_mode_t *port_egvlanmode)
{
	...
	if (FAL_FLG_TST(port_egvlanmode->mask, FAL_EGRESSMODE_CTAG_EN)) {
		...
		if (ADPT_IS_VPORT(port_id)) {
			...
			if (adpt_chip_type_get(dev_id) == CHIP_HTTPPE) {
#if defined(HTTPPE)
				rtn = adpt_httppe_port_vlantag_egmode_set(dev_id, port_id, 0, value, FAL_EGRESSMODE_CTAG_EN);
				SW_RTN_ON_ERROR(rtn);
#endif
			} else {
	...
	if (FAL_FLG_TST(port_egvlanmode->mask, FAL_EGRESSMODE_STAG_EN)) {
		...
		if (ADPT_IS_VPORT(port_id)) {
			...
			if (adpt_chip_type_get(dev_id) == CHIP_HTTPPE) {
#if defined(HTTPPE)
				rtn = adpt_httppe_port_vlantag_egmode_set(dev_id, port_id, value, 0, FAL_EGRESSMODE_STAG_EN);
				SW_RTN_ON_ERROR(rtn);
#endif
			} else {
...
{
	sw_error_t rtn = SW_OK;
	a_uint32_t stag_mode = 0, ctag_mode = 0;
	a_uint32_t port_value = FAL_PORT_ID_VALUE(port_id);
	...
	if (ADPT_IS_VPORT(port_id)) {
		...
		if (adpt_chip_type_get(dev_id) == CHIP_HTTPPE) {
#if defined(HTTPPE)
			rtn = adpt_httppe_port_vlantag_egmode_get(dev_id, port_id, &stag_mode, &ctag_mode);
			SW_RTN_ON_ERROR(rtn);
#endif
		} else {
	...
	rtn = adpt_portvlan_egmode_convert(&port_egvlanmode->stag_mode, &stag_mode, A_FALSE);
	SW_RTN_ON_ERROR(rtn);

	rtn = adpt_portvlan_egmode_convert(&port_egvlanmode->ctag_mode, &ctag_mode, A_FALSE);
	SW_RTN_ON_ERROR(rtn);
                                 ...
                                 fal_fwd_cmd_t cmd)
{
	...
	if (ADPT_IS_VPORT(port_id)) {
		...
		if (adpt_chip_type_get(dev_id) == CHIP_HTTPPE) {
#if defined(HTTPPE)
			rtn = adpt_httppe_port_vlan_xlt_miss_fwd_cmd_set(dev_id, port_id, value);
#endif
		} else {
                                 ...
                                 fal_fwd_cmd_t *cmd)
{
	...
	if (ADPT_IS_VPORT(port_id)) {
		...
		if (adpt_chip_type_get(dev_id) == CHIP_HTTPPE) {
#if defined(HTTPPE)
			rtn = adpt_httppe_port_vlan_xlt_miss_fwd_cmd_get(dev_id, port_id, &value);
#endif
		} else {
		...
		fal_port_t port_id, a_bool_t enable)
{
	...
	if (ADPT_IS_VPORT(port_id)) {
		...
		if (adpt_chip_type_get(dev_id) == CHIP_HTTPPE) {
#if defined(HTTPPE)
			rtn = adpt_httppe_port_vlantag_vsi_egmode_enable_set(dev_id, port_id, enable);
#endif
		} else {
		...
		fal_port_t port_id, a_bool_t * enable)
{
	...
	} else
	{
		if (adpt_chip_type_get(dev_id) == CHIP_HTTPPE) {
#if defined(HTTPPE)
			rtn = adpt_httppe_port_vlantag_vsi_egmode_enable_get(dev_id, port_id, enable);
#endif
		} else {
		...
		fal_vlan_trans_adv_rule_t *rule, fal_vlan_trans_adv_action_t *action)
{
	...
	for (idx = 0; idx < FAL_PORTVLAN_TRANS_TBL_NUM(direction); idx++) {
		...
		if (!_get_port_vlan_trans_adv_rule_by_index(dev_id, idx, direction, &tmp_rule, &tmp_action))
			continue;

		/* Check if rule and action are same */
		if (!_check_if_rule_equal(direction, &tmp_rule, rule) &&
			!_check_if_action_equal(direction, &tmp_action, action)) {
...
adpt_hppe_portvlan_member_update(a_uint32_t dev_id, fal_port_t port_id, fal_pbmp_t mem_port_map)
{
	...
	if(ADPT_IS_VPORT(port_id))
	{
		if (adpt_chip_type_get(dev_id) == CHIP_HTTPPE)
#if defined(HTTPPE)
			return adpt_httppe_portvlan_vpmember_update(dev_id, port_id, mem_port_map);
#endif
		else
...
adpt_hppe_portvlan_member_get(a_uint32_t dev_id, fal_port_t port_id, fal_pbmp_t * mem_port_map)
{
	...
	if(ADPT_IS_VPORT(port_id))
	{
		if (adpt_chip_type_get(dev_id) == CHIP_HTTPPE)
#if defined(HTTPPE)
			return adpt_httppe_portvlan_vpmember_get(dev_id, port_id, mem_port_map);
#endif
		else
...
adpt_hppe_qinq_mode_set(a_uint32_t dev_id, fal_qinq_mode_t mode)
{
	...
	if (adpt_chip_type_get(dev_id) == CHIP_HTTPPE) {
#if defined(HTTPPE)
		rtn = adpt_httppe_global_qinq_mode_set(dev_id, &global_mode);
#endif
	} else {
		rtn = adpt_hppe_global_qinq_mode_set(dev_id, &global_mode);
	}
...
adpt_hppe_qinq_mode_get(a_uint32_t dev_id, fal_qinq_mode_t * mode)
{
	...
	if (adpt_chip_type_get(dev_id) == CHIP_HTTPPE) {
#if defined(HTTPPE)
		rtn = adpt_httppe_global_qinq_mode_get(dev_id, &global_mode);
		SW_RTN_ON_ERROR(rtn);
#endif
	} else {
		rtn = adpt_hppe_global_qinq_mode_get(dev_id, &global_mode);
		SW_RTN_ON_ERROR(rtn);
	}
...
adpt_hppe_port_qinq_role_set(a_uint32_t dev_id, fal_port_t port_id, fal_qinq_port_role_t role)
{
	...
	if (adpt_chip_type_get(dev_id) == CHIP_HTTPPE) {
#if defined(HTTPPE)
		rtn = adpt_httppe_port_qinq_mode_set(dev_id, port_id, &port_role);
#endif
	} else {
		rtn = adpt_hppe_port_qinq_mode_set(dev_id, port_id, &port_role);
	}
...
adpt_hppe_port_qinq_role_get(a_uint32_t dev_id, fal_port_t port_id, fal_qinq_port_role_t * role)
{
	...
	if (adpt_chip_type_get(dev_id) == CHIP_HTTPPE) {
#if defined(HTTPPE)
		rtn = adpt_httppe_port_qinq_mode_get(dev_id, port_id, &port_role);
		SW_RTN_ON_ERROR(rtn);
#endif
	} else {
		rtn = adpt_hppe_port_qinq_mode_get(dev_id, port_id, &port_role);
		SW_RTN_ON_ERROR(rtn);
	}
		...
		fal_port_default_vid_enable_t *default_vid_en, fal_port_vlan_tag_t *default_tag)
{
	...
	if (direction == FAL_PORT_VLAN_ALL || direction == FAL_PORT_VLAN_INGRESS)
	{
		if (ADPT_IS_VPORT(port_id)) {
			...
			if (adpt_chip_type_get(dev_id) == CHIP_HTTPPE) {
#if defined(HTTPPE)
				rtn = adpt_httppe_port_default_pcp_set(dev_id, port_id, default_tag);
				SW_RTN_ON_ERROR(rtn);
#endif
			} else {
				if (FAL_FLG_TST(default_tag->mask, FAL_PORT_VLAN_TAG_CPCP_EN)) {
		...
		fal_port_default_vid_enable_t *default_vid_en, fal_port_vlan_tag_t *default_tag)
{
	...
	if (direction == FAL_PORT_VLAN_INGRESS) {
		if (ADPT_IS_VPORT(port_id)) {
			...
			if (adpt_chip_type_get(dev_id) == CHIP_HTTPPE) {
#if defined(HTTPPE)
				rtn = adpt_httppe_port_default_pcp_get(dev_id, port_id, default_tag);
				SW_RTN_ON_ERROR(rtn);
#endif
			} else {
...
adpt_hppe_portvlan_member_add(a_uint32_t dev_id, fal_port_t port_id, fal_port_t mem_port_id)
{
	...
	if(ADPT_IS_VPORT(port_id))
	{
		if (adpt_chip_type_get(dev_id) == CHIP_HTTPPE)
#if defined(HTTPPE)
			return adpt_httppe_portvlan_vpmember_add(dev_id, port_id, mem_port_id);
#endif
		else
...
adpt_hppe_portvlan_member_del(a_uint32_t dev_id, fal_port_t port_id, fal_port_t mem_port_id)
{
	...
	if(ADPT_IS_VPORT(port_id))
	{
		if (adpt_chip_type_get(dev_id) == CHIP_HTTPPE)
#if defined(HTTPPE)
			return adpt_httppe_portvlan_vpmember_del(dev_id, port_id, mem_port_id);
#endif
		else
		...
		fal_ingress_vlan_filter_t *filter)
{
	...
	if (ADPT_IS_VPORT(port_id)) {
		...
		if (adpt_chip_type_get(dev_id) == CHIP_HTTPPE) {
#if defined(HTTPPE)
			rtn = adpt_httppe_port_ingress_vlan_filter_set(dev_id, port_id, filter);
			SW_RTN_ON_ERROR(rtn);
#endif
		} else {
		...
		fal_ingress_vlan_filter_t *filter)
{
	...
	if (ADPT_IS_VPORT(port_id)) {
		...
		if (adpt_chip_type_get(dev_id) == CHIP_HTTPPE) {
#if defined(HTTPPE)
			rtn = adpt_httppe_port_ingress_vlan_filter_get(dev_id, port_id, filter);
			SW_RTN_ON_ERROR(rtn);
#endif
		} else {
		...
		fal_vlantag_propagation_t *prop)
{
	...
	if (direction == FAL_PORT_VLAN_ALL || direction == FAL_PORT_VLAN_INGRESS)
	{
		if (ADPT_IS_VPORT(port_id)) {
			...
			if (adpt_chip_type_get(dev_id) == CHIP_HTTPPE) {
#if defined(HTTPPE)
				rtn = adpt_httppe_port_tag_propagation_set(dev_id, port_id, prop->mask,
					FAL_PORT_VLAN_INGRESS, value_pcp, value_dei);
				SW_RTN_ON_ERROR(rtn);
#endif
			} else {
				if (FAL_FLG_TST(prop->mask, FAL_PORT_PROPAGATION_PCP_EN)) {
...
#if defined(HTTPPE)
				rtn = adpt_httppe_port_tag_propagation_set(dev_id, port_id, prop->mask,
					FAL_PORT_VLAN_EGRESS, value_pcp, value_dei);
				SW_RTN_ON_ERROR(rtn);
#endif
			} else {
				if (FAL_FLG_TST(prop->mask, FAL_PORT_PROPAGATION_PCP_EN)) {
		...
		fal_vlantag_propagation_t *prop)
{
	...
	if (direction == FAL_PORT_VLAN_INGRESS) {
		if (ADPT_IS_VPORT(port_id)) {
			...
			if (adpt_chip_type_get(dev_id) == CHIP_HTTPPE) {
#if defined(HTTPPE)
				rtn = adpt_httppe_port_tag_propagation_get(dev_id, port_id,
					FAL_PORT_VLAN_INGRESS, &value_pcp, &value_dei);
				SW_RTN_ON_ERROR(rtn);
#endif
			} else {
		...
		a_uint32_t cnt_index, fal_port_vlan_counter_t * counter)
{
	...
	if (adpt_chip_type_get(dev_id) == CHIP_HTTPPE) {
#if defined(HTTPPE)
		rtn = adpt_httppe_port_vlan_counter_get(dev_id, cnt_index, counter);
#endif
	} else {
...
adpt_hppe_port_vlan_counter_cleanup(a_uint32_t dev_id, a_uint32_t cnt_index)
{
	...
	if (adpt_chip_type_get(dev_id) == CHIP_HTTPPE) {
#if defined(HTTPPE)
		rtn = adpt_httppe_port_vlan_counter_cleanup(dev_id, cnt_index);
#endif
	} else {
		union vlan_dev_tx_counter_tbl_u vlan_dev_tx_counter_tbl;
...
sw_error_t adpt_hppe_portvlan_init(a_uint32_t dev_id)
{
	...
	if (adpt_chip_type_get(dev_id) == CHIP_HTTPPE) {
		...
		p_adpt_api->adpt_port_isol_group_get = adpt_httppe_port_isol_group_get;
		p_adpt_api->adpt_port_isol_action_ctrl_set = adpt_httppe_port_isol_action_ctrl_set;
