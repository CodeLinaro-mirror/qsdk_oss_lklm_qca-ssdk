...
typedef enum
{
	FAL_FRAME_LEN_ORIGINAL = 0, /* original length */
	FAL_FRAME_LEN_DECAP,        /* length after decap */
	FAL_FRAME_LEN_OUTPUT        /* output length */
} fal_policer_length_mode_t;
...
typedef struct
{
	...
	fal_policer_length_mode_t length_mode;/*Length mode for rate limit calculation, add it for ipq96xx */
